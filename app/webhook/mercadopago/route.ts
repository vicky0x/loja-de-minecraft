...
async function assignProductsToUser(order: any, db: mongoose.mongo.Db) {
  try {
    ...
    for (const item of order.items) {
      ...
      logger.info(`Encontrados ${stockItems.length} itens disponíveis para atribuição (necessários: ${item.quantity})`);
      
      // Verificar se há itens suficientes
      if (stockItems.length < item.quantity) {
        logger.error(`Estoque insuficiente para o item ${item.name || item.productId}. Encontrado: ${stockItems.length}, Necessário: ${item.quantity}`);
        continue;
      }
