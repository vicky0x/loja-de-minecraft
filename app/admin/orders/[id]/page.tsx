...
export default function OrderDetailPage() {
  ...
  const fetchOrderData = async () => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        const errorMessage = extractErrorFromResponse(textResponse);
        setError(`Erro ao obter dados do pedido: ${errorMessage}`);
        setLoading(false);
        return;
      }
      ...
      if (!response.ok) {
        ...
        setError(`Falha ao carregar pedido: ${errorMessage}`);
        setLoading(false);
        return;
      }
      ...
      if (!validateOrderData(data)) {
        setError('Dados do pedido incompletos ou inválidos');
        setLoading(false);
        return;
      }
      ...
      if (err.name === 'AbortError') {
        setError('Tempo limite excedido ao buscar dados do pedido');
      } else {
  ...
  const validateOrderData = (data) => {
    ...
    const requiredFields = ['id', 'status', 'createdAt', 'items'];
    for (const field of requiredFields) {
      if (data[field] === undefined) {
        console.error(`Campo obrigatório ausente nos dados do pedido: ${field}`);
        return false;
      }
    }
    ...
    if (!Array.isArray(data.items) || data.items.length === 0) {
      console.error('Pedido não contém itens');
      return false;
    }
    
    // Verificar dados de pagamento
    if (data.status !== 'pending' && !data.paymentInfo) {
      console.warn('Pedido não-pendente sem informações de pagamento');
      // Ainda retorna true, pois não é um erro fatal
    }
    
    return true;
  };
  
  // Função para extrair mensagens de erro de respostas HTML ou texto
  const extractErrorFromResponse = (responseText) => {
    if (!responseText || responseText.trim() === '') {
      return 'Resposta vazia do servidor';
    }

    try {
      // Tentar encontrar mensagens de erro em tags HTML comuns
      let errorMatch = responseText.match(/<div class="error">(.*?)<\/div>/i) ||
        responseText.match(/<p class="error">(.*?)<\/p>/i) ||
        responseText.match(/<div class="message error">(.*?)<\/div>/i) ||
        responseText.match(/<h1>(.*?Error:.*?)<\/h1>/i) ||
        responseText.match(/<title>(Error:.*?)<\/title>/i);

      if (errorMatch && errorMatch[1]) {
        // Limpar tags HTML internas e códigos de entidade
        return errorMatch[1].trim();
      }
      
      // Verificar se o texto é potencialmente JSON com formato incorreto
      if (responseText.includes('"error":') || responseText.includes('"message":')) {
        try {
          // Tentar encontrar um objeto JSON dentro do texto
          const jsonMatch = responseText.match(/(\{.*\})/);
          if (jsonMatch && jsonMatch[1]) {
            const parsedJson = JSON.parse(jsonMatch[1]);
            return parsedJson.error || parsedJson.message || null;
          }
        } catch (e) {
          console.warn('Falha ao extrair JSON da resposta:', e);
        }
      }
      
      // Se o texto for muito longo, retornar apenas um trecho
      if (responseText.length > 150) {
        return `Erro do servidor: ${responseText.substring(0, 147)}...`;
      }
      
      // Retornar o texto completo se for curto
      return responseText;
    } catch (e) {
    ...
    return (
      <div className="container mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-white">Detalhes do Pedido</h1>
          <Link 
            href="/admin/orders" 
            className="inline-flex items-center text-sm bg-dark-300 hover:bg-dark-400 text-white py-2 px-4 rounded"
          >
        ...
        <div className="bg-dark-200 rounded-lg shadow-md p-8 text-center">
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="bg-dark-200 rounded-lg shadow-md p-8 text-center">
          <div className="bg-red-900/30 border-l-4 border-red-500 p-4 text-red-400 flex items-start mb-6 mx-auto max-w-md">
            <FiAlertCircle className="mr-2 mt-0.5 flex-shrink-0" size={20} />
            <div>
              <p className="font-medium mb-1">Erro ao carregar detalhes do pedido</p>
          ...
          <div className="flex flex-col items-center mt-4 space-y-3">
            <button
              ...
              className="bg-primary hover:bg-primary-dark text-white font-medium py-2 px-6 rounded-md flex items-center"
            >
            ...
            <button
              ...
              className="text-red-400 hover:text-red-500 text-sm mt-6 flex items-center"
            >
  ...
  if (!order) {
    return (
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="bg-dark-200 rounded-lg shadow-md p-8 text-center">
          <div className="bg-yellow-900/30 border-l-4 border-yellow-500 p-4 text-yellow-400 flex items-start mb-6 mx-auto max-w-md">
            <FiAlertTriangle className="mr-2 mt-0.5 flex-shrink-0" size={20} />
            <p>Pedido não encontrado ou você não tem permissão para visualizá-lo.</p>
          </div>
          
          <Link
            href="/admin/orders"
            className="bg-primary hover:bg-primary-dark text-white font-medium py-2 px-6 rounded-md inline-flex items-center mt-4"
          >
            <FiArrowLeft className="mr-2" />
            Voltar para Lista de Pedidos
          </Link>
